...
def test_save_and_get_download(tmp_path):
    with patch("idm_cli.downloader.state.STATE_DIR", str(tmp_path)), patch(
        "idm_cli.downloader.state.STATE_FILE", str(tmp_path / "state.json")
    ):
        save_download(
            "task1",
            "http://example.com",
...
# Test extractors
from idm_cli.extractors.direct import (
    extract_urls,
    fetch_all_info,
