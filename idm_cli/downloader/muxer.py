            ...
            if _is_windows:
                with zipfile.ZipFile(archive_path, "r") as zip_ref:
                    candidate = next(
                        (
                            ...
                            if item.filename.endswith("ffmpeg.exe")
                        ),
                        None,
                    )
                ...
                with tarfile.open(archive_path, "r:xz") as tar_ref:
                    candidate = next(
                        (
                            item
                            for item in tar_ref.getmembers()
                            if item.name.endswith("/ffmpeg") or item.name == "ffmpeg"
                        ),
                        None,
                    )
                    if candidate is None or not candidate.isfile():
                        raise RuntimeError(
                            "FFmpeg archive did not contain a regular ffmpeg binary."
                        )
                    source = tar_ref.extractfile(candidate)
                    if source is None:
                        raise RuntimeError("Could not read FFmpeg binary from archive.")
                    with source:
                        async with aiofiles.open(target_path, "wb") as target:
                            while data := source.read(1024 * 1024):
                                await target.write(data)
            if not os.path.isfile(target_path) or os.path.getsize(target_path) == 0:
                raise RuntimeError("Downloaded FFmpeg binary is empty.")
    finally:
        try:
...
def mux_audio_video(video_path: str, audio_path: str, output_path: str) -> None:
    ...
    try:
        ...
        logger.error(
            f"FFmpeg muxing failed: {e.stderr.decode('utf-8', errors='replace')}"
        )
...
def convert_to_mp3(audio_path: str, output_path: str) -> None:
    ...
    try:
        ...
        logger.error(
            f"FFmpeg conversion failed: {e.stderr.decode('utf-8', errors='replace')}"
        )
